...
exports.check_plain_passwd_dn = async function (connection, user, passwd, cb) {
  ...
  // Try each candidate DN in parallel; succeed on the first match. We use
  // Promise.any by turning a `false` result into a rejection — Promise.any
  // resolves with the first fulfillment, so we get early success without
  // waiting for the slower attempts to time out.
  try {
    const ok = await Promise.any(
      dns.map(async (dn) => {
        const verified = await this._verify_user(
          dn.replace(/%u/g, escapeDN(user)),
          passwd,
          connection,
        )
        if (!verified) throw new Error('not this dn')
        return true
      }),
    )
    cb(ok)
  } catch {
    // AggregateError: every candidate either failed or returned false
    cb(false)
  }
}
